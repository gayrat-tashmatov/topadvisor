      ...
      <div className="container-wide section-padding">
        ...
        <div className="flex justify-between items-end mb-14">
          ...
          <a
            ...
            className="hidden md:inline-flex items-center gap-2 text-sm text-gold-500 hover:text-gold-600 font-medium transition-colors"
          >
            {t('view_all')}
            <ArrowRight size={16} />
          </a>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {sampleInsights.map((insight, i) => (
            <a
              ...
              className="group glass-card p-6 hover:border-gold-400/40 transition-all duration-500 hover:-translate-y-1 hover:shadow-lg flex flex-col"
            >
              {/* Type + date */}
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-2xs text-navy-300">{insight.date}</span>
              </div>

              {/* Title */}
              <h3 className="font-display text-lg font-bold text-navy-800 group-hover:text-gold-500 transition-colors mb-3 leading-snug">
                {locale === 'en' ? insight.titleEn : insight.titleRu}
              </h3>

              {/* Excerpt */}
              <p className="text-sm text-navy-400 leading-relaxed mb-5 flex-1">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-navy-50">
                <div className="flex items-center gap-1.5 text-2xs text-navy-300">
                  <Clock size={12} />
                  {insight.readTime}
                </div>
                <span className="text-xs font-semibold text-gold-400 group-hover:text-gold-500 transition-colors">
                  {t('read_more')} →
                </span>
              </div>
            </a>
        ...
        <div className="md:hidden mt-8 text-center">
          <a href={`/${locale}/insights`} className="inline-flex items-center gap-2 text-sm text-gold-500">
